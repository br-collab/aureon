...
STATE_FILE = os.path.join(THIS_DIR, "aureon_state_persist.json")
LOG_FILE   = os.path.join(THIS_DIR, "aureon_errors.log")
...
def api_decisions():
    ...
    with _lock:
        return jsonify({
            ...
            "count":   len(aureon_state["pending_decisions"]),
            "ts":      datetime.now(timezone.utc).isoformat(),
        })
...
def api_resolve_decision(decision_id):
    ...
    with _lock:
        if aureon_state["halt_active"] and resolution == "APPROVED":
            return jsonify({
                "error": "SYSTEM HALTED — execution blocked. Resume system before approving trades.",
    ...
    with _lock:
        ...
        decision = next(
            (d for d in aureon_state["pending_decisions"] if d["id"] == decision_id),
            None
        )
        if not decision:
        ...
        # Create human authority log entry with a unique hash
        # This hash is the immutable audit fingerprint for this decision
        authority_hash = hashlib.sha256(
            f"{decision_id}{resolution}".encode()
        ).hexdigest()[:16].upper()
        ...
        log_entry = {
            ...
            "ts":        datetime.now(timezone.utc).isoformat(),
            "tier":      "Tier 1",
            "type":      f"Trade {resolution}: {decision['action']} {decision['symbol']}",
            "authority": "br@ravelobizdev.com",
            "outcome":   resolution,
            "hash":      authority_hash,
        }
        ...
        if resolution == "APPROVED":
            ...
            # ── Email trade confirmation (non-blocking) ──
            threading.Thread(
            ...
            print(f"[AUREON] REJECTED: {decision['action']} {decision['symbol']} "
                  f"— hash {authority_hash}")
    ...
    report_id_out = None
    if resolution == "APPROVED":
        r = aureon_state["trade_reports"][0] if aureon_state["trade_reports"] else {}
        report_id_out = r.get("report_id")
...
def api_pretrade_check(decision_id):
    ...
    # ── Gate 1: Session boundary (Verana L0)
    if action == "BUY" and not is_crypto and not market_open:
        session_status = "WARN"
        session_msg    = f"Pre-market window — {symbol} queued for open (9:30 ET)"
    else:
        session_status = "PASS"
        session_msg    = "Market session valid" if market_open else "Crypto — 24/7 session"

    # ── Gate 2: Liquidity buffer (Risk Manager L1B)
    if action == "BUY":
        ...
        if cash < notional:
            ...
            cash_status = "WARN"
            cash_msg    = (f"Trade would breach 3% operating floor — "
                           f"post-trade cash ${post_trade_cash:,.0f} < floor ${cash_floor:,.0f}")
        elif cash >= notional:
            cash_status = "PASS"
            cash_msg    = (f"Cash available: ${cash:,.0f} — order ${notional:,} covered — "
                           f"floor ${cash_floor:,.0f} maintained")
        else:
            cash_status = "WARN"
            cash_msg    = f"Cash tight: ${cash:,.0f} vs ${notional:,} notional"
    else:
        cash_status = "PASS"
        cash_msg    = "SELL — no cash requirement"

    # ── Gate 3: Drawdown guard (Risk Manager L1B)
    if drawdown > RISK_MANAGER_POLICY["drawdown_fail_pct"]:
        dd_status = "FAIL"
        dd_msg    = f"Drawdown {drawdown:.2f}% exceeds {RISK_MANAGER_POLICY['drawdown_fail_pct']:.0f}% threshold — BUY blocked"
    elif drawdown > RISK_MANAGER_POLICY["drawdown_warn_pct"]:
        dd_status = "WARN"
    ...
    # ── Gate 4: Position concentration (Risk Manager L1B)
    pos_pct = (notional / portfolio_value * 100) if portfolio_value > 0 else 0
    if pos_pct > RISK_MANAGER_POLICY["position_fail_pct"]:
        pos_status = "FAIL"
        pos_msg    = f"Position {pos_pct:.1f}% of portfolio exceeds {RISK_MANAGER_POLICY['position_fail_pct']:.0f}% hard cap"
    elif pos_pct > RISK_MANAGER_POLICY["position_warn_pct"]:
        pos_status = "WARN"
    ...
    # ── Gate 5: Doctrine integrity (Thifur L3)
    doc_status = "PASS"
    doc_msg    = "Doctrine v1.2 — audit lineage intact"

    # ── Gate 6: Systemic stress overlay (Verana L0 / OFR)
    ...
    # ── Gate 7: OFAC / Sanctions Screening (Verana L0 — Compliance Layer)
    # Check symbol against the simulated SDN / blocked-party list.
    # In production: Thomson Reuters World-Check or OFAC SDN XML feed (daily refresh).
    ...
    if isin_id and isin_id in OFAC_BLOCKED_ISINS:
        ...
        ofac_msg    = (f"SDN clear — {symbol} not on OFAC blocked list "
                       f"(screened {screen_date} · source: OFAC SDN XML)")
    ...
    # Settlement window
    # SEC Rule 15c6-1 amendment (effective May 28, 2024): US equities settle T+1.
    # Crypto settles T+0 (on-chain finality). FX and fixed income remain T+1.
    settlement = "T+0" if is_crypto else "T+1"

    # Cache gate results so api_resolve_decision can embed them in the compliance report
    with _lock:
        aureon_state.setdefault("_pretrade_cache", {})[decision_id] = {
            "overall": overall, "checks": checks, "settlement": settlement,
            "ofr": ofr_snapshot, "macro": macro_snapshot,
            "ts": datetime.now(timezone.utc).isoformat(),
        }
...
def start():
    print("=" * 60)
    print("  PROJECT AUREON — The Grid 3")
    print("  Doctrine-Driven Financial Operating System")
    print("=" * 60)
    print()
    ...
    print("  Snapshot   →  http://localhost:5001/api/snapshot")
    print("  Portfolio  →  http://localhost:5001/api/portfolio")
    print("  Decisions  →  http://localhost:5001/api/decisions")
    print()
...
# "if this script is run directly (not imported), call start()"
