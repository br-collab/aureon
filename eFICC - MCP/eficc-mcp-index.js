...
/**
 * eFICC MCP Server
 * Free public data sources for Fixed Income, Currencies & Commodities
 *
 ...
 *   - BLS:           CPI, PPI (macro regime context)
 */
...
async function get(url, params = {}) {
  try {
    const res = await axios.get(url, { params, timeout: 10000,
      headers: { "User-Agent": "eFICC-MCP-Server/1.0 (open-source; contact: github)" }
    });
    return res.data;
  } catch (e) {
...
const TOOLS = [
  ...
  // ── DSOR GOVERNANCE TOOLS ─────────────────────────────────────────────────
  {
    name: "get_ficc_context",
    description: "Consolidated eFICC governance context for DSOR pre-trade record: SOFR, 10y yield, 2y10y spread, OFR stress index, fed funds. Single tool for Aureon pre-trade doctrine gate assembly.",
    inputSchema: { type: "object", properties: {} }
  }
];
...
async function handleTool(name, args) {
  switch (name) {
    ...
    case "get_macro_regime_snapshot": {
      ...
      const t3m = parseFloat(t3m.observations?.[0]?.value || 0);
      ...
      return {
        ...
        spreads: {
          ...
          "3m10y_bps": ((t10 - parseFloat(t3m.observations?.[0]?.value || 0)) * 100).toFixed(1)
        },
        macro: {
    ...
    case "get_recent_13f_filers": {
      ...
      const data = await get(`${EDGAR_BASE}/submissions/`, {});
      // EDGAR full-text search for 13F filings
      const search = await get(
        `${EDGAR_BASE}/efts/v1/hits.json`,
        { q: '"13F-HR"', dateRange: "custom", startdt: cutoff, forms: "13F-HR", hits: { hits: { total: {}, _source: {} } } }
      );
      return {
        source: "SEC EDGAR",
    ...
    case "get_ficc_context": {
      ...
      return {
        ...
        description: "Pre-trade DSOR governance context — eFICC regime snapshot",
        rates: {
          sofr: sofr.observations?.[0],
          treasury_10y: t10y.observations?.[0],
          treasury_2y: t2y.observations?.[0],
...
async function main() {
  ...
  process.stderr.write("eFICC MCP Server running — 17 tools across NY Fed, FRED, TreasuryDirect, OFR, SEC EDGAR\n");
}
